...
//            response.sendRedirect("/login");
        ...
        try {
            ...
            FoodDTO foodDTO = FoodService.INSTANCE.getFoodWithUuidService(getUuid);
            ...
            // 회원정보를 , 세션에 추가하기.
            session.setAttribute("loginInfo", foodDTO);
            //계속 필터 동작을 진행하겠다.
...
//            String result  = (String) session.getAttribute("loginInfo");
            FoodDTO foodDTO  = (FoodDTO) session.getAttribute("loginInfo");
            log.info("session.getAttribute(\"loginInfo\") foodDTO : " + foodDTO);
        }
        filterChain.doFilter(servletRequest, servletResponse);
    }
